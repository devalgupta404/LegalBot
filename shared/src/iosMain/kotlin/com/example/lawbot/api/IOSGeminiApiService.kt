...
class IOSOpenRouterApiService : GeminiApiService {
    ...
    override suspend fun sendMessage(message: String, apiKey: String): String {
        ...
        for (attempt in 1..maxRetries) {
            try {
                ...
                println("iOS: Sending request to OpenRouter API (attempt $attempt/$maxRetries) with model: ${ApiConfig.OPENROUTER_MODEL}")
                
                val response: OpenRouterResponse = client.post("${ApiConfig.OPENROUTER_BASE_URL}/api/v1/chat/completions") {
                    header("Authorization", "Bearer $apiKey")
                    header("HTTP-Referer", "https://lawbot.app")
                    header("X-Title", "LawBot")
                    contentType(ContentType.Application.Json)
                    setBody(request)
                }.body()
                
                println("iOS: Received response on attempt $attempt: $response")
                ...
                lastException = e
                println("iOS: API Error on attempt $attempt: ${e.message}")
                ...
                if (attempt < maxRetries && isRetryable) {
                    ...
                    println("iOS: Retrying in ${delayMs}ms...")
